...
interface AgregarPaginaEstaticaModalProps {
  isOpen: boolean
  onClose: () => void
  onAdd: (data: PaginaEstaticaResponse) => void
}
...
export const AgregarPaginaEstaticaModal = ({ 
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Error al crear página estática:", error)
      toast({
        title: "Error",
        description: error.message || "Error al crear la página estática",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)

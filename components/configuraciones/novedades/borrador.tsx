...
interface NovedadBorrador {
  id: string
  nombre: string
  estado: "Borrador"
}

export function NovedadesBorrador() {
  ...
  // Datos de ejemplo para novedades en borrador
  const NOVEDADES_BORRADOR_DATA: NovedadBorrador[] = [
    {
      id: "5",
      nombre: "Novedad 5",
      estado: "Borrador"
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedItems(NOVEDADES_BORRADOR_DATA.map(item => item.id))
    } else {
      setSelectedItems([])
    }
  ...
  const handleRefresh = () => {
    console.log("Refrescando novedades en borrador...")
  }

  const handleDeleteSelected = () => {
    console.log("Eliminando novedades en borrador seleccionadas:", selectedItems)
  }
  ...
  const handleEditarNovedad = (id: string) => {
    const novedad = NOVEDADES_BORRADOR_DATA.find(n => n.id === id)
    if (novedad) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tabla de novedades en borrador */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full table-fixed">
            ...
            <tbody className="bg-[#FBFBFB]">
              {NOVEDADES_BORRADOR_DATA.map((novedad, index) => (
                <tr 
                  key={novedad.id} 
                  className="bg-[#FBFBFB]"
                  style={{ 
                    borderBottom: index < NOVEDADES_BORRADOR_DATA.length - 1 ? '1px solid #A4A4A4' : 'none'
                  }}
                >

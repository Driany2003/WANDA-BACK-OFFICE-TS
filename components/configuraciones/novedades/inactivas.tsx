...
export function NovedadesInactivas() {
  ...
  // Datos de ejemplo para novedades inactivas
  const NOVEDADES_INACTIVAS_DATA: NovedadInactiva[] = [
    {
      id: "2",
      nombre: "Novedad 2",
      estado: "Inactivo"
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedItems(NOVEDADES_INACTIVAS_DATA.map(item => item.id))
    } else {
      setSelectedItems([])
    }
  ...
  const handleRefresh = () => {
    console.log("Refrescando novedades inactivas...")
  }
  ...
  const handleEditarNovedad = (id: string) => {
    const novedad = NOVEDADES_INACTIVAS_DATA.find(n => n.id === id)
    if (novedad) {
        ...
        <div className="overflow-x-auto">
          <table className="w-full table-fixed">
            ...
            <tbody className="bg-[#FBFBFB]">
              {NOVEDADES_INACTIVAS_DATA.map((novedad, index) => (
                <tr 
                  key={novedad.id} 
                  className="bg-[#FBFBFB]"
                  style={{ 
                    borderBottom: index < NOVEDADES_INACTIVAS_DATA.length - 1 ? '1px solid #A4A4A4' : 'none'
                  }}
                >

...
interface EnProcesoSalidasProps {
  onViewDetail: (subscriber: any) => void
}

export function EnProcesoSalidas({ onViewDetail }: EnProcesoSalidasProps) {
  const [selectedItems, setSelectedItems] = useState<string[]>([])
  const [selectAll, setSelectAll] = useState(false)

  // Filtrar solo los registros en proceso
  const inProcessData = WITHDRAWAL_DATA.filter(item => item.estado === "En proceso")

  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedItems(inProcessData.map(item => item.id))
    } else {
      setSelectedItems([])
    }
        ...
        <div className="overflow-x-auto">
          <table className="w-full table-fixed">
            ...
            <tbody className="bg-[#FBFBFB]">
              {inProcessData.map((item, index) => (
                <tr 
                  key={item.id} 
                  className="bg-[#FBFBFB]"
                  style={{ 
                    borderBottom: index < inProcessData.length - 1 ? '1px solid #A4A4A4' : 'none'
                  }}
                >

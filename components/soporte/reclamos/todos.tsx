...
export function Todos() {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedReclamos(RECLAMOS_DATA.map(reclamo => reclamo.id))
    } else {
      setSelectedReclamos([])
    }
  }
  ...
  const isAllSelected = selectedReclamos.length === RECLAMOS_DATA.length
      ...
      <div className="overflow-hidden rounded-xl bg-white" style={{ boxShadow: '0 4px 20px rgba(219, 8, 110, 0.15)' }}>
        <div className="p-3 sm:p-4 md:p-6">
          ...
          <div className="space-y-0">
            {RECLAMOS_DATA.map((reclamo, index) => (
              <div key={reclamo.id}>

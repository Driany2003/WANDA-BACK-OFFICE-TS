...
// Mock data para reclamos en proceso
const RECLAMOS_EN_PROCESO = [
  {
    id: '0087',
    mensaje: 'Reclamo 0087: Demora en el retiro de dinero al usuario @Jesús21...',
    estado: 'En proceso',
    fecha: '2024-01-15 14:30'
  },
  {
    id: '0092',
    mensaje: 'Reclamo 0092: Problema con la validación de documentos...',
    estado: 'En proceso',
    fecha: '2024-01-16 11:45'
  }
]
...
export function EnProceso() {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedReclamos(RECLAMOS_EN_PROCESO.map(reclamo => reclamo.id))
    } else {
      setSelectedReclamos([])
    }
  }
  ...
  const isAllSelected = selectedReclamos.length === RECLAMOS_EN_PROCESO.length
      ...
      <div className="overflow-hidden rounded-xl bg-white" style={{ boxShadow: '0 4px 20px rgba(219, 8, 110, 0.15)' }}>
        <div className="p-3 sm:p-4 md:p-6">
          ...
          <div className="space-y-0">
            {RECLAMOS_EN_PROCESO.map((reclamo, index) => (
              <div key={reclamo.id}>
                ...
                {index < RECLAMOS_EN_PROCESO.length - 1 && (
